...
const {breadthFirstTraverseRecursively, depthFirstInsertIteratively, createNode} = require("../src/df-insert-bf-traverse");

// const {breadthFirstTraverse, depthFirstInsert, createNode} = require("../src/df-insert-bf-traverse");

// const {breadthFirstTraverseRecursively, depthFirstInsert, createNode} = require("../src/df-insert-bf-traverse");

// const {breadthFirstTraverse, depthFirstInsertIteratively, createNode} = require("../src/df-insert-bf-traverse");

const insertWithString = (str, root = createNode("8")) => {
  str.split("").forEach((c) => depthFirstInsertIteratively(root, c));
  return root;
};
...
for (let i = 0; i < testCases.length; i++) {
  ...
  test(`Insert "${str}", Traverse should be: "${traversedExpected}"`, () => {
    ...
    let traversed = breadthFirstTraverseRecursively(root).join("")
